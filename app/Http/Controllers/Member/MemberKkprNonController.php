...
class MemberKkprNonController extends Controller
{
    ...
    public function edit($id)
    {
        try {
            ...
            $relationships = ['kkpr_koordinat', 'kkpr_terbit'];
            if (empty($model->kbli)) {
            ...
            \Illuminate\Support\Facades\Log::error('Member KKPR Non Edit Error: ' . $e->getMessage(), [
                'id' => $id,
                'user_id' => Auth::id(),
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            Koordinat_kkpr::where('id_kkpr', $model->id)->where('jenis', 'UMK')->delete();
            ...
            return redirect()->route('member.kkprnon.edit', $id)
                ->with('success', 'Data berhasil diupdate kedalam sistem');
        } catch (\Exception $e) {
            DB::rollBack();
            
            \Illuminate\Support\Facades\Log::error('Member KKPR Non Update Error: ' . $e->getMessage(), [
                'id' => $id,
                'user_id' => Auth::id(),
                'trace' => $e->getTraceAsString()
            ]);
