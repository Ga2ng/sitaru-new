...
@section('title', 'SITARU - KKPR Terbit Otomatis')
@section('subtitle', 'Penilaian untuk Persetujuan KKPR Terbit Otomatis')
    ...
    <div class="relative overflow-hidden bg-gradient-to-br from-[#185B3C] via-[#0F3D26] to-[#185B3C] rounded-xl p-6 text-white">
        ...
        <div class="relative z-10">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold mb-1">KKPR Terbit Otomatis</h1>
                    <p class="text-sm text-white/90 mb-4">Untuk dokumen KKPR yang diterbitkan secara otomatis melalui OSS</p>
                ...
                <div class="px-6 py-4 hover:bg-gradient-to-r hover:from-[#185B3C]/5 hover:to-transparent transition-all duration-300 group">
                    <div class="grid grid-cols-12 gap-4 items-center">
                        ...
                        <div class="col-span-1">
                                ...
                                @endphp
                            <button id="btn-aksi-{{ $kkpr->id }}" onclick="toggleDropdown(event, {{ $kkpr->id }}, {{ $kkpr->proses }}, {{ $kkpr->revisi }}, {{ $can['verifikator'] ? 'true' : 'false' }}, {{ $can['analis'] ? 'true' : 'false' }}, {{ $can['pimpinan'] ? 'true' : 'false' }}, {{ $can['kabid'] ? 'true' : 'false' }}, {{ $can['kepala_dinas'] ? 'true' : 'false' }}, {{ $can['upload_draft'] ? 'true' : 'false' }}, {{ $can['opd_eksternal'] ? 'true' : 'false' }}, {{ $can['tim_fpr'] ? 'true' : 'false' }}, {{ $kkpr->deleted ?? 0 }}, {{ $kkpr->f_survey ? '\'' . route('admin.kkpr.view.survey', $kkpr->id) . '\'' : 'null' }})" class="inline-flex items-center space-x-1.5 px-3 py-1.5 text-xs font-medium text-gray-700 bg-white hover:bg-[#185B3C]/10 hover:text-[#185B3C] border border-gray-300 rounded-lg transition-all duration-200 hover:scale-105" title="Aksi">
...
<script>
    ...
    function renderRiwayat(riwayat, model) {
        ...
        subtitle.textContent = (model.jenis == 'non_umk' ? 'KKPR Terbit Otomatis' : 'KKPR') + ' #' + model.id;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (surveyForm) {
            surveyForm.addEventListener('submit', function(event) {
                ...
                fetch(`/admin/kkpr/survey/${currentSurveyId}`, {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                    },
    ...
    function kirimKabid(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch(`/admin/kkpr/kirim-kabid/${id}`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
<div id="upload-draft-modal" style="display: none; position: fixed; top: 0; right: 0; bottom: 0; left: 0; z-index: 9999; overflow-y: auto;">
    <div style="display: flex; min-height: 100%; align-items: center; justify-content: center; padding: 1rem;">
        <div style="position: relative; background-color: #ffffff; border-radius: 1rem; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.25); max-width: 32rem; width: 100%; transform: translateY(0); transition: all 0.15s;">
            ...
            <!-- Modal Body -->
            <form id="upload-draft-form" action="{{ route('admin.kkpr.upload.draft') }}" method="POST" enctype="multipart/form-data" style="padding: 1.5rem;">
                @csrf
