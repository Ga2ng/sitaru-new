            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-white/90">Ubah data {{ $user->name }}</p>
                    </div>
                </div>
            ...
            <form action="{{ route('admin.users.update', $user) }}" method="POST" class="space-y-6">
                ...
                <div class="bg-gray-50 rounded-lg p-4">
                    ...
                    <div class="mt-4">
                        ...
                        <input type="text" name="work" value="{{ old('work', $user->work) }}" 
                    ...
                    <div class="mt-4">
                        ...
                        <textarea name="address" rows="3" 
                                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#185B3C] focus:border-transparent @error('address') border-red-500 @enderror">{{ old('address', $user->address) }}</textarea>
                        @error('address')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mt-4">
                        ...
                        <input type="text" name="ktp" value="{{ old('ktp', $user->ktp) }}" 
                    ...
                    <div class="mt-4">
                        ...
                        <select name="role_id" 
                            ...
                            @foreach($roles as $id => $name)
                                <option value="{{ $id }}" {{ old('role_id', $user->roles->first()->id ?? '') == $id ? 'selected' : '' }}>
                                    {{ $name }}
                                </option>
                            @endforeach
                        </select>
                            ...
                            @foreach($permissions as $id => $name)
                                <label class="flex items-center">
                                    <input type="checkbox" name="permission_lists[]" value="{{ $id }}" 
                                           {{ in_array($id, old('permission_lists', $user->permissions->pluck('id')->toArray())) ? 'checked' : '' }}
