...
@section('title', 'Persetujuan Dokumen KKPR Non')
@section('subtitle', 'Persetujuan hasil analisa dokumen KKPR Non #' . $model->id)
...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="relative overflow-hidden bg-gradient-to-br from-[#185B3C] via-[#0F3D26] to-[#185B3C] rounded-xl p-6 text-white">
        ...
        <div class="relative z-10">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold mb-1">Persetujuan Dokumen KKPR Non #{{ $model->id }}</h1>
    ...
    <div class="flex flex-wrap gap-4">
        ...
        <a href="{{ route('admin.kkprnon.show', $model->id) }}" class="inline-flex items-center px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors">
    ...
    <div class="bg-white/80 backdrop-blur-sm rounded-xl p-6 shadow-lg border border-white/20">
        <div class="flex items-center space-x-3 mb-6">
            ...
            <h3 class="text-lg font-bold text-gray-900">1. Kesesuaian Kegiatan Pemanfaatan Ruang</h3>
        </div>
        ...
        <div class="space-y-4">
            ...
            <div class="space-y-2">
                ...
                <div class="border-2 border-blue-200 rounded-xl overflow-hidden">
                    <img src="{{ url('uploads/berkas/umk/' . $model->id . '/peta/' . $model->foto_peta) }}" 
                         alt="Foto Peta" 
                         class="w-full h-auto"
...
<script>
    ...
    function loadGeoJSON(map) {
        const geojsonUrl = '{{ url("uploads/berkas/umk/" . $model->id . "/kml/" . ($model->f_geojson ?? "")) }}';
    ...
    function revisiDokumen(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                fetch('{{ route("admin.kkprnon.persetujuan.revisi") }}', {
                    method: 'POST',
                    headers: {
    ...
    function setujuiDokumen(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                fetch('{{ route("admin.kkprnon.persetujuan.setuju") }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                        'Content-Type': 'application/json',
